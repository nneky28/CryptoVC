...
const DonatorModal = ({ onClose, isOpen, project }) => {
  ...
  const handleDonate = async () => {
    ...
    if (typeof window.ethereum !== 'undefined') {
      try {
        const tx = await crowdfundingFunctions.pledge(1, amt);
        await tx.wait();
